...
class ChatGroup:
    ...
    @classmethod
    def create(cls, group_id: int) -> "ChatGroup":
        conn = get_connection()
        cursor = conn.cursor()
        # гарантируем, что запись только одна
        cursor.execute("DELETE FROM chat_group")
        cursor.execute("INSERT INTO chat_group (group_id) VALUES (?)", (group_id,))
        conn.commit()

        cursor.execute("SELECT id, group_id FROM chat_group LIMIT 1")
        row = cursor.fetchone()
        conn.close()
        return cls(*row)
    ...
    @classmethod
    def update(cls, group_id: int) -> "ChatGroup":
        conn = get_connection()
        cursor = conn.cursor()
